...
use App\Models\ParliamentModel;
use Config\Database;
...
class Parliament extends BaseController
{
    use ResponseTrait;
     ...
     public function index()
    {
       ...
       $db = $tenantService->getTenantConfig($this->request->getHeaderLine('X-Tenant-Config')); // Load UserModel with the tenant database connection
        $ParliamentModel = new ParliamentModel($db);
        return $this->respond(["status" => true, "message" => "All Data Fetched", "data" => $ParliamentModel->findAll()], 200);
    }

   
public function getAllParliamentPaging()
{
    $input = $this->request->getJSON();

    $page = isset($input->page) ? (int)$input->page : 1;
    $perPage = isset($input->perPage) ? (int)$input->perPage : 10;
    $sortField = isset($input->sortField) ? $input->sortField : 'parliamentConstituencyId';
...
public function create()
{
    ...
    // Validation rules for parliament fields
    $rules = [
        'constituencyCode'      => ['rules' => 'required|numeric'],
    ...
    $tenantService = new TenantService();
    $db = $tenantService->getTenantConfig($this->request->getHeaderLine('X-Tenant-Config'));
    $model = new ParliamentModel($db); // Make sure you have a ParliamentModel
    $id = $model->insert($data);

    return $this->respond([
        'status' => true,
        'message' => 'Parliament constituency added successfully',
        'data' => $id
    ], 200);
}
