    ...
    public function getPosPaging()
{
    ...
    public function getPos()
    {
        ...
        if (!empty($filter)) {
            ...
            foreach ($filter as $key => $value) {
                if (in_array($key, ['poCode', 'poDate', 'taxInvoiceNumber', 'businessNameFor', 'phoneFor', 'addressFor', 'emailFor', 'total', 'totalItem', 'totalPrice', 'createdBy', 'modifiedBy', 'createdDate', 'modifiedDate'])) {
        ...
        public function createPos()
    {
        ...
        if ($this->validate($rules)) {
            ...
            $db = $tenantService->getTenantConfig($this->request->getHeaderLine('X-Tenant-Config'));
            $model = new PoModel($db);
            $PoModel = $model->insert($input);
            return $this->respond(["status" => true, "message" => "Brand Added Successfully", "data" => $PoModel], 200);
        }else{
            $response = [
                'status' => false,
                'errors' => $this->validator->getErrors(),
