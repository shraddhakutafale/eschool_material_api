...
class Blog extends BaseController
{
    ...
    public function getBlogsPaging()
    {
        ...
        // Load UserModel with the tenant database connection
        $blogModel = new BlogModel($db);
        $blogs = $blogModel->orderBy('createdDate', 'DESC')->paginate($perPage, 'default', $page);
    ...
    public function getBlogsWebsite()
    {
        // Retrieve tenantConfig from the headers
        $tenantConfigHeader = $this->request->getHeaderLine('X-Tenant-Config');
        if (!$tenantConfigHeader) {
    ...
    public function create()
    {
        ...
        $rules = [
            ...
            'blogTitle' => ['rules' => 'required'],
            'blogAuthor' => ['rules' => 'required'],
            'blogTags' => ['rules' => 'required'],
            'blogContent' => ['rules' => 'required'],
    ...
    public function update()
    {
        ...
        $rules = [
            'blogId' => ['rules' => 'required|numeric'], // Ensure courseId is provided and is numeric
        ];
        ...
        if ($this->validate($rules)) {
            ...
            $blogId = $input->blogId;
            $blog = $model->find($blogId); // Assuming find method retrieves the course
    ...
    public function delete()
    {
        ...
        // Validation rules for the course
        $rules = [
            'blogId' => ['rules' => 'required'], // Ensure eventId is provided and is numeric
        ];
    ...
    public function uploadPageProfile()
    {
        ...
        $blogId = $this->request->getPost('blogId'); // Example field
    ...
    public function getBlogById($blogId)
{
    ...
    // Fetch course by ID
    $blog = $blogModel->find($blogId); // find method returns a single record by its ID
    ...
    return $this->respond(["status" => true, "message" => "Sale fetched successfully", "data" => $blog], 200);
}
